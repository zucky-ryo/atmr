<?php
    ...
    if($_GET['sw'] == "show_page"){
        ...
        $html .= sprintf("</div>");
        $html .= sprintf("<div class='d-flex justify-content-center align-items-center scroll_x mb-2 px-2'>");
        $arr = ["A","あ","か","さ","た","な","は","ま","や","ら","わ"];
        for($i=0;$i<11;$i++){
            $html .= sprintf("<button type='button' class='mx-1 px-2 btn btn btn-outline-dark text-center'>%s</button>",$arr[$i]);
        }
        $html .= sprintf("</div>");
