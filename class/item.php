<?php
    ...
    class Item extends Database{
        ...
        public function getItems($user_id,$type,$s_type){
            ...
            $sql .= sprintf("where type='%d' %s order by case when TRIM(kana) regexp '^[0-9]' then 1 when TRIM(kana) regexp '^[a-z]' then 2 else 3 end desc,",$type,$s_type);
            $sql .= sprintf("replace(replace(kana, 'ー', 'あ'),'ヴ','ウ') ASC");
            $res = mysqli_query($this->conn,$sql);
